    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-900 to-blue-900 text-white flex flex-col items-center justify-center p-6">
        ...
        <p className="text-gray-300 text-center max-w-md text-lg">
          Add your email as "Parent Email" when creating a child's account to see their learning journey here! ✨
        </p>
        <a
          href="/"
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {children.map((child) => (
              <button
                ...
                className={isSelected(child._id)
                ...
                <div className="relative p-6">
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      ...
                      />
                      {isSelected(child._id) && (
                        <div className="absolute -top-2 -right-2 bg-yellow-400 text-purple-900 rounded-full p-1">⭐</div>
                      )}
                    </div>
                    
                    <div className="text-left flex-1">
                      <h3 className="font-bold text-lg">{child.fullName}</h3>
                      <p className={isSelected(child._id) ? "text-sm text-purple-100" : "text-sm text-gray-300"}>
                        🎂 Age {child.age}
                      </p>
                    </div>
                    
                    <div className="text-center bg-black/20 rounded-xl p-3 backdrop-blur-sm">
                      <div className="text-3xl font-black">{child.stats.totalVideosWatched}</div>
                      <div className="text-xs uppercase font-semibold">videos</div>
                    </div>
                  </div>
                  ...
                  {isSelected(child._id) && (
                    <div className="mt-4 pt-4 border-t border-white/20 grid grid-cols-3 gap-2 text-center text-xs">
                      <div>
                        <div className="font-bold text-lg">{child.stats.totalWatchTime}</div>
                        <div className="text-purple-100">mins</div>
                      </div>
                      <div>
                        <div className="font-bold text-lg">{child.stats.completedVideos}</div>
                        <div className="text-purple-100">done</div>
                      </div>
                      <div>
                        <div className="font-bold text-lg">{child.stats.completionRate}%</div>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          {[
            ...
            { id: "progress", label: "Learning Journey", icon: "📚", color: "from-purple-500 to-pink-500" },
        ...
        {overview && activeTab === "overview" && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { icon: "📺", label: "Videos Watched", value: overview.summary.totalVideosWatched, desc: "Total amazing videos! 🎬", color: "from-blue-500/30 to-cyan-500/30", border: "blue-400/50" },
                { icon: "⏱️", label: "Watch Time", value: overview.summary.totalWatchTime, desc: "Minutes of learning! ⏰", color: "from-green-500/30 to-emerald-500/30", border: "green-400/50" },
                { icon: "✅", label: "Completed", value: overview.summary.completedVideos, desc: "Videos finished! 🎉", color: "from-purple-500/30 to-pink-500/30", border: "purple-400/50" },
                { icon: "📊", label: "Completion Rate", value: `${overview.summary.averageCompletionRate}%`, desc: "Average finish rate! 🎯", color: "from-yellow-500/30 to-orange-500/30", border: "yellow-400/50" },
              ].map((stat, idx) => (
                <div key={idx} className={`bg-gradient-to-br ${stat.color} backdrop-blur-lg p-8 rounded-3xl border-2 border-${stat.border} shadow-2xl transform hover:scale-105 transition`}>
                  <div className="text-6xl mb-4 animate-bounce">{stat.icon}</div>
                  <div className="text-sm text-white/80 mb-2 font-semibold uppercase">{stat.label}</div>
                  ...
                  return (
                    <div key={index} className="group">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="text-sm text-gray-400">{day.videosWatched} videos 🎬</div>
                      </div>
                      ...
                      <div className="relative bg-gray-800/50 rounded-full h-12 overflow-hidden border border-purple-500/30">
                        ...
                        >
                          {day.watchTime > 0 && (
                            <span className="text-white font-bold text-sm drop-shadow-lg">{day.watchTime} min 🔥</span>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {overview.topCategories.map((category, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`bg-gradient-to-br ${colors[index % colors.length]} bg-opacity-20 backdrop-blur-lg p-6 rounded-2xl border-2 border-white/20 shadow-xl transform hover:scale-105 transition`}
                    >
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="font-black text-xl text-white">{category.name}</h4>
                      ...
                      <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div className="bg-black/20 rounded-xl p-3 backdrop-blur-sm">
                          <div className="text-2xl font-bold">{category.totalTime}m</div>
        ...
        {/* PROGRESS TAB */}
        {activeTab === "progress" && selectedChild && <WatchHistory childId={selectedChild} />}
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-gradient-to-br from-orange-500/30 to-red-500/30 backdrop-blur-xl p-10 rounded-3xl border-4 border-orange-400/50 shadow-2xl text-center transform hover:scale-105 hover:rotate-1 transition">
                ...
                <div className="text-2xl font-bold text-orange-200 mb-2">Day Streak</div>
                <div className="text-lg text-orange-100">Keep the fire burning! 🚀</div>
              ...
              <div className="bg-gradient-to-br from-yellow-500/30 to-amber-500/30 backdrop-blur-xl p-10 rounded-3xl border-4 border-yellow-400/50 shadow-2xl text-center transform hover:scale-105 hover:rotate-1 transition">
                <div className="text-8xl mb-6 animate-bounce" style={{ animationDelay: "0.2s" }}>
                  🏆
                </div>
                <div className="text-7xl font-black mb-4 text-white drop-shadow-2xl">
                  {achievements.longestStreak}
                </div>
                <div className="text-2xl font-bold text-yellow-200 mb-2">Longest Streak</div>
                <div className="text-lg text-yellow-100">Personal Best Record! 👑</div>
                {achievements.currentStreak === achievements.longestStreak && achievements.currentStreak > 0 && (
                  <div className="mt-4 bg-yellow-400 text-purple-900 px-4 py-2 rounded-full font-bold inline-block animate-pulse">
                    🔥 AT PEAK PERFORMANCE! 🔥
                  </div>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-green-500/30 to-emerald-500/30 backdrop-blur-xl p-8 rounded-3xl border-2 border-green-400/50 shadow-2xl">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-green-200 text-sm mb-2 font-semibold uppercase">Videos Completed</div>
                    <div className="text-6xl font-black text-white">{achievements.totalVideosCompleted}</div>
                    <div className="text-green-200 mt-2">Videos fully finished! 🎬</div>
                  </div>
              ...
              <div className="bg-gradient-to-br from-blue-500/30 to-cyan-500/30 backdrop-blur-xl p-8 rounded-3xl border-2 border-blue-400/50 shadow-2xl">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-blue-200 text-sm mb-2 font-semibold uppercase">Total Learning Time</div>
                    <div className="text-6xl font-black text-white">{achievements.totalWatchTime}</div>
                    <div className="text-blue-200 mt-2">Minutes of knowledge! ⏰</div>
                  </div>
            ...
            <div className="bg-white/5 backdrop-blur-xl p-8 rounded-3xl border-2 border-white/10 shadow-2xl">
              ...
              {achievements.badges.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {achievements.badges.map((badge, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`bg-gradient-to-br ${colors[index % colors.length]} backdrop-blur-xl p-8 rounded-3xl border-4 border-white/30 shadow-2xl text-center transform hover:scale-110 hover:rotate-3 transition cursor-pointer group`}
                      >
                        <div className="text-7xl mb-4 group-hover:animate-bounce">{badge.icon}</div>
                        <h4 className="font-black text-2xl mb-3 text-white drop-shadow-lg">{badge.name}</h4>
                        <p className="text-white/90 font-semibold">{badge.description}</p>
                ...
                <div className="text-center py-16">
                  <div className="text-8xl mb-6 animate-bounce">🎯</div>
                  <h3 className="text-3xl font-bold text-white mb-4">No Badges Yet!</h3>
                  <p className="text-xl text-gray-300 mb-6">Keep learning to unlock awesome achievements! 🚀</p>

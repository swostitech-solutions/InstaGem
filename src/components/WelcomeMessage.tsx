    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md mx-4 p-8 transform animate-scaleIn">
        <div className="text-center">
          {/* Animated emoji */}
          <div className="text-7xl mb-4 animate-bounce">{ageInfo.emoji}</div>

          {/* Gradient text */}
          <h2
            className={`text-3xl font-bold mb-3 bg-gradient-to-r ${ageInfo.color} bg-clip-text text-transparent`}
          >
          ...
          {/* Age group badge */}
          <div
            className={`inline-block px-4 py-2 rounded-full bg-gradient-to-r ${ageInfo.color} text-white font-semibold mb-4`}
          >
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 mb-6">
            <p className="text-sm text-gray-700">
              {user.childAge <= 4 &&
                "🎈 Get ready to explore colors, shapes, and stories!"}
              {user.childAge >= 5 &&
                user.childAge <= 8 &&
                "📚 Time to learn science, math, and amazing facts!"}
              {user.childAge >= 9 &&
                user.childAge <= 12 &&
                "🔬 Dive into experiments, history, and cool discoveries!"}
              {user.childAge >= 13 &&
                "🎓 Explore advanced topics, projects, and inspiring ideas!"}
            </p>
          </div>
